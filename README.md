...
| 目录 / 文件 | 说明 |
|---|---|
| [`momentum_strategy/`](momentum_strategy/) | **推荐**。分层的独立项目：配置 / 指标 / 数据源 / 选股 / 择时 / 撮合 / 引擎 / 统计分离，带 204 个单元测试，可用合成数据离线跑通 |
| `dl_strategy_xtdata.py` | 单文件版（原策略口径），**由 `momentum_strategy/tools/build_standalone.py` 自动生成**，适合直接丢进 QMT 目录 |
| `dl_strategy_opt.py` | 单文件版（优化版口径：5 只等权 + 仓位 35% + 择时盯持仓 + 回看 29 天 + 涨停按开盘价拦截），同样自动生成 |
...
python dl_strategy_xtdata.py --eval-scores results/scores_xxx.csv        # 榜单：次日开盘买入、第二天开盘卖出
```
